(function () {
    ...
    servingUomMaster.$inject = ['lodash', '$scope', '$translate', 'DTOptionsBuilder'];

    /* @ngInject */
    function servingUomMaster(_, $scope, $translate, DTOptionsBuilder) {
        ...
        function edit() {
            ...
            delete vm.selectedServingUom.isSelected;
            vm.selectedServingUoms = [];
            vm.selectedServingUom = null;
        }
        ...
        function toggleSelection(item) {
            var index = vm.selectedServingUoms.indexOf(item);
            item.isSelected = !item.isSelected;

            if (item.isSelected) {
                vm.selectedServingUom = item;

                // If item isn't in the selected items array...
                if (index < 0) {
                    // add it.
                    vm.selectedServingUoms.push(item);
                }
            } else {
                // If item isn't in the selected items array...
                if (index > -1) {
