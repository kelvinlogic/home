(function () {
    ...
    angular
        ...
        .controller('ProductAttributeMasterCtrl', productAttributeMaster);

    productAttributeMaster.$inject = ['lodash', '$scope', '$translate', 'DTOptionsBuilder'];

    /* @ngInject */
    function productAttributeMaster(_, $scope, $translate, DTOptionsBuilder) {
        ...
        vm.selectedProductAttributes = [];
        vm.selectedProductAttribute = null;
        vm.productAttribute = null;
        vm.productAttributes = null;
        vm.titleKey = 'fc.merchandising.productAttribute.MASTER_PAGE_TITLE';
        ...
        function edit() {
            ...
            vm.productAttribute = angular.extend({}, vm.selectedProductAttribute);

            // Clear table selection
            delete vm.selectedProductAttribute.isSelected;
            vm.selectedProductAttributes = [];
            vm.selectedProductAttribute = null;
        }
        ...
        function toggleSelection(item) {
            var index = vm.selectedProductAttributes.indexOf(item);
            item.isSelected = !item.isSelected;

            if (item.isSelected) {
                vm.selectedProductAttribute = item;

                // If item isn't in the selected items array...
                if (index < 0) {
                    // add it.
                    vm.selectedProductAttributes.push(item);
                }
            } else {
                // If item isn't in the selected items array...
                if (index > -1) {
                    // remove it.
                    vm.selectedProductAttributes.splice(index, 1);
                }

                vm.selectedProductAttribute = _.last(vm.selectedProductAttributes);
            }
        }
    }
})();