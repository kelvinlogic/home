(function () {
    ...
    salesmanMaster.$inject = ['lodash', '$scope', '$translate', 'DTOptionsBuilder'];

    /* @ngInject */
    function salesmanMaster(_, $scope, $translate, DTOptionsBuilder) {
        ...
        vm.selectedSalesmen = [];
        vm.selectedSalesman = null;
        vm.salesman = null;
        vm.salesmen = [];
        vm.titleKey = 'fc.merchandising.salesman.MASTER_PAGE_TITLE';
        ...
        function edit() {
            ...
            vm.salesman = angular.extend({}, vm.selectedSalesman);

            // Clear table selection
            delete vm.selectedSalesman.isSelected;
            vm.selectedSalesmen = [];
            vm.selectedSalesman = null;
        }
        ...
        function toggleSelection(item) {
            var index = vm.selectedSalesmen.indexOf(item);
            item.isSelected = !item.isSelected;

            if (item.isSelected) {
                vm.selectedSalesman = item;

                // If item isn't in the selected items array...
                if (index < 0) {
                    // add it.
                    vm.selectedSalesmen.push(item);
                }
            } else {
                // If item isn't in the selected items array...
                if (index > -1) {
                    // remove it.
                    vm.selectedSalesmen.splice(index, 1);
                }

                vm.selectedSalesman = _.last(vm.selectedSalesmen);
            }
        }
    }
})();