...
global.inMemDatabase.credits = {};

var db = global.inMemDatabase.credits;
...
router.get("/credits", function (req, resp) {
    ...
    var credits = _(db).values();

    if (sortField) {
        credits.sortBy(function (item) {
        ...
        if (filter_on_fields) {
            _.forEach(filter_on_fields, function (field) {
                if (_.has(req.query, field)) {
                    credits = credits.filter(function (entity) {
    ...
    if (!isTrueRegEx.test(req.query["showInactive"])) {
        credits = credits.filter({"active": true});
    }

    var inlineCount = credits.size();

    credits = credits.rest(skip).first(getFirst);
